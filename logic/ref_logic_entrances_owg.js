/**
 * ref_logic_entrances_owg.js — entrance logic with glitches allowed.
 *
 * Copied from alttptracker-main/data/logic/owg/logic_entrances.js with one
 * change: the global it assigns, so the map can hold this set and the
 * no-glitch one at once and swap between them by setting.
 *
 * Hybrid Major Glitches shares it — upstream owg/logic_entrances.js and
 * hmg/logic_entrances.js are byte-identical (checked Sep 2026); the two modes
 * only diverge in logic_nondungeon_checks and the dungeon logic, neither of
 * which the map consumes. It differs from the no-glitch set in 106 lines.
 */
(function (window) {
  "use strict";

  window.logic_entrances_owg = {
    "20 Rupee Cave": {
      Open: {
        allOf: ["glove"],
      },
